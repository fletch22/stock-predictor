...
class TestEquityUtilService(TestCase):
  ...
  def test_get_todays_merged_shar_data(self):
    ...
    youngest = df_merged.iloc[-1,:]['date']

    logger.info(f"Last date: {youngest}")
  ...
  def test_get_average_volatility(self):
    end_date = date_utils.parse_datestring("2019-09-21")
    df = EquityUtilService.get_merged_mean_stdev(end_date, 253)
